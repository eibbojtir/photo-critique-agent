...
from photo_critique_agent.models.critique import AnalysisOptions, CritiqueContext, CritiqueResult
...
class MetadataAssessment:
    """Structured outcome of metadata scoring before prose rendering."""
...
class MetadataPlaceholderEvaluator(CritiqueEvaluator):
    ...
    def evaluate(
        ...
        assessment = _assess_metadata(asset)
        strengths = list(assessment.strengths)
        weaknesses = list(assessment.weaknesses)

        if assessment.rating:
            strengths.append(f"Existing rating context: {assessment.rating}.")
        if assessment.keywords:
            strengths.append(f"Keywords emphasize {', '.join(assessment.keywords[:3])}.")
        if active_options.style:
            strengths.append(
                f"Style study lens applied: {active_options.style}, with feedback nudged toward that artist's visual priorities."
            )

        if assessment.recommendation == "keep":
            strengths.append(
...
def _assess_metadata(asset: PhotoAsset) -> MetadataAssessment:
    exif = asset.exif
    ...
    rating = _normalize_optional_text(supplemental_values.get("rating"))
...
def _build_critique_paragraph(
    *,
    ...
    details: list[str] = [
        f"This placeholder {persona.name} critique scores the frame at {assessment.score:.1f}/10 and lands on a {assessment.recommendation} recommendation.",
    ]
    if focal_length is not None:
    ...
    if assessment.rating:
        details.append(f"CSV context includes a prior rating of {assessment.rating}.")
    if assessment.keywords:
        details.append(f"Keywords noted for this frame: {', '.join(assessment.keywords)}.")
    if style:
        details.append(
